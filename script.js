...
function getActualMachineName(){
  var lastSession = sessions[sessions.length -1]
  if(lastSession.user != -1)
    return machines[lastSession.machine].name;
  else
    return "_"
}
...
function executeCommand(event){
  ...
  if(event.keyCode==13){
    ...
    if (sessions[0].user === -1){
      ...
      document.getElementById('command').value=response.command; 
      ...
      document.getElementById('command').value=getPrepend(); 
    }
...
function login(command){
  ...
  if(command[0].trim() == 'Login'){
    if(command.length < 2){
      return {
        message:'<span class="highlighted">Error: Login: usuario</span>',
        command: login_label
      }
    ...
    logged_user = users.findIndex(user => user.name == command[1].trim());
    if(logged_user  != -1){
  ...
  return {
      message:'<span class="highlighted">Error: Fallo en el registro</span>',
      command: login_label
    }
}


document.getElementById('command').value=login_label;
